...
getDayData_manual=function(day,siteInfoPath,outType){
  period=1
  ...
  deps=sqlQuery(TempDB,paste0("SELECT DISTINCT DeploymentIDX FROM Data WHERE ABS(DateDiff('d', DateTimeLocal, #",format(formatDate,format="%Y-%m-%d"),"# ))<",period," AND MetricIDX=1"))$DeploymentIDX
    ...
    for(i in 1:length(deps)){
      ...
      output=rbind(output,
                   data.frame(
                     ...
                     mean=mean(sqlQuery(TempDB,paste0("SELECT Observation, DateTimeLocal FROM Data WHERE ABS(DateDiff('d', DateTimeLocal, #",format(formatDate,format="%Y-%m-%d"),"# ))<",period," AND MetricIDX=1 AND DeploymentIDX=",deps[i]))$Observation),
                     min=min(sqlQuery(TempDB,paste0("SELECT Observation, DateTimeLocal FROM Data WHERE ABS(DateDiff('d', DateTimeLocal, #",format(formatDate,format="%Y-%m-%d"),"# ))<",period," AND MetricIDX=1 AND DeploymentIDX=",deps[i]))$Observation),
                     max=max(sqlQuery(TempDB,paste0("SELECT Observation, DateTimeLocal FROM Data WHERE ABS(DateDiff('d', DateTimeLocal, #",format(formatDate,format="%Y-%m-%d"),"# ))<",period," AND MetricIDX=1 AND DeploymentIDX=",deps[i]))$Observation),
      ...
      dayData=sqlQuery(TempDB,paste0("SELECT Observation,DateTimeLocal FROM Data WHERE ABS(DateDiff('d', DateTimeLocal, #",format(formatDate,format="%Y-%m-%d"),"# ))<",period," AND MetricIDX=1 AND DeploymentIDX=",deps[i]))
      names(dayData)[1]=paste0("dep",deps[i])
      if(i==1){
        allDayData=dayData
      }else{
        allDayData=merge(dayData,allDayData,by='DateTimeLocal')
      }
    }
