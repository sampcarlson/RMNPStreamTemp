...
lines(fitDF$day,fitDF$med_stream,lwd=4,col="white")
lines(fitDF$day,fitDF$med_lake,lwd=4,col="white")

lines(fitDF$day,fitDF$med_stream,lwd=1.5,lty=1)
lines(fitDF$day,fitDF$med_lake,lwd=1.5,lty=2)
...
legend(x="topright",legend=c("Stream Elevation individual fit","5-day Median Stream Elevation fit","Lake Elevation individual fit","5-day Median Lake Elevation fit"),
       pch=c(16,NA,17,NA),lty=c(NA,1,NA,2),lwd=c(1,1.5,1,1.5), 
       bty="n")
dev.off()
